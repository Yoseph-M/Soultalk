            ...
            <div className="flex-1 flex flex-col h-full relative min-w-0">
                ...
                <header className={`px-4 py-3 flex items-center justify-between border-b backdrop-blur-md sticky top-0 z-20 ${isDark ? 'bg-[#0f172a]/90 border-slate-800' : 'bg-white/90 border-slate-200'
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center shadow-lg text-white ${primaryColor}`}>
                                <PenBox className="w-4 h-4 md:w-5 md:h-5" />
                            </div>
                            <h1 className="font-bold text-xs md:text-base">Mindful Journal</h1>
                        </div>
                    </div>
                    {isProfessional && (
                        <div className="relative group max-w-[150px] md:max-w-none">
                            <select
                                ...
                                className={`pl-8 md:pl-10 pr-4 py-1.5 md:py-2 rounded-xl text-[10px] md:text-xs font-bold border appearance-none transition-all cursor-pointer ring-offset-2 focus:ring-2 focus:ring-[#F97316] ${isDark
                            ...
                            >
                                ...
                                {clients.map((conn) => (
                                    <option key={conn.id} value={conn.client}>{conn.client_name}</option>
                                ))}
                            </select>
                            <UserIcon className="absolute left-2.5 md:left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#F97316]" />
                        </div>
                    )}
                </header>
                    ...
                    <div className="max-w-4xl mx-auto w-full">
                        ...
                        <div className="mb-6 md:mb-8">
                            <input
                                ...
                                className={`w-full bg-transparent text-2xl md:text-5xl font-bold tracking-tight focus:outline-none placeholder:opacity-20 ${isDark ? 'text-white' : 'text-slate-900'} ${!title && isSaving ? 'placeholder:text-red-400' : ''} ${editingEntryId ? 'cursor-default' : ''}`}
                            />
                            <div className={`w-16 md:w-20 h-1 mt-3 md:mt-4 rounded-full ${primaryColor}`} />
                        </div>
                            ...
                            <div className={`flex flex-wrap gap-2 mb-8`}>
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all border ${mode === m.id
                                            ? `bg-[#F97316] text-white border-[#F97316] shadow-sm`
                                            : `text-slate-500 hover:bg-[#F97316]/10 hover:text-[#F97316] ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`
                                            }`}
                                    >
                            ...
                            <>
                                {(title || entry || recordedBlob) && (
                                    <button
                                        onClick={startNewEntry}
                                        disabled={isSaving}
                                        className="flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl font-bold transition-all text-xs md:text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-30"
                                    >
                                ...
                                <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-2 px-6 md:px-8 py-2.5 md:py-3 rounded-xl font-bold transition-all shadow-lg text-xs md:text-sm text-white ${saved ? 'bg-green-500' : (isSaving || (mode === 'text' ? !entry.trim() : !recordedBlob)) ? 'bg-slate-400 cursor-not-allowed opacity-50' : primaryColor
                                        }`}
                                >
